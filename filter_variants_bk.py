...
def __main__():
    ...
    filter_variants(variant_summary, filters, pairing, filtered_summary, somatic_summary)
    ...
    if (os.path.isfile(somatic_summary)):
        ...
        var_patients_occur = out[1]
        gene_variants_occur = out[2]
        calculate_occurrence(somatic_summary, gene_patients_occur, var_patients_occur, gene_variants_occur) 
        os.reomve(somatic_summary)

    # remove intermediate files
    os.remove(filtered_summary)
def filter_variants(variant_summary, filters, pairing, filtered_summary, somatic_summary):
    ...
    with open (filtered_summary,  'wb') as fh:
        ...
        with open (variant_summary, 'r') as handle:
             ...
             if (pairing == 'paired'):
                 print('Filtering variant with matching normal!')
                 for line in records:
                     gene = line['gene']
                     chr = line["chromosome"]
                     pos = line["position"]
                 ...
                 print('Filtering variant without matching normal!')
                 for line in records:
                     gene = line['gene']
                     chr = line["chromosome"]
                     pos = line["position"]
